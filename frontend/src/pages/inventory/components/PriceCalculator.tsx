...
export function PriceCalculator({ basePrice: initialBase = 0, floor: initialFloor = 1, readOnly = false, onPricingChange }: Props) {
  ...
  const snapshot = useMemo<PricingSnapshot>(() => {
    ...
    const lineItems = [
      { label: "Base Price", amount: baseVal },
    ...
    const s: PricingSnapshot = { basePrice: baseVal, floorRise: floorRiseTotal, plc: plcVal, parking: parkingVal, otherCharges: otherVal, gstRate: gst, subtotal, gstAmount, total, lineItems };
    onPricingChange?.(s);
    return s;
  }, [base, floor, floorRisePerFloor, plc, parking, otherCharges, gstRate]);
